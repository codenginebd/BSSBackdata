...
class FBCrawler:
	def __init__(self):
		pass
	def FindSingleTypeFavorites(self,link,checkLogin=False):
		self.browser.OpenURL(link)
		if checkLogin is True:
			passwordBoxInFacebook = self.browser.CheckIfPasswordElementExistsInFacebook()
			if passwordBoxInFacebook is not None:
	...
	def CrawlProfile(self,browser,parser,facebookBasicProfile):
		...
		booksItems = self.FindSingleTypeFavorites(booksLink,checkLogin=True)
		if booksItems == "LOGIN_REQUIRED":
			return "LOGIN_REQUIRED"
